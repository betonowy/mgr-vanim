...
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){ 5,  6,  7}", &dst, cubes, 5, 6, 7);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){-3, -1, -2}", &dst, cubes, -3, -1, -2);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){ 5,  6,  0}", &dst, cubes, 5, 6, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){-3, -1,  0}", &dst, cubes, -3, -1, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){ 5,  0,  0}", &dst, cubes, 5, 0, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){-3,  0,  0}", &dst, cubes, -3, 0, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(f32){ 0,  0,  0}", &dst, cubes, 0, 0, 0);
...
// TEST_CASE("rotate_refill_speed_mask")
// {
//     static constexpr glm::vec3 high_coord{24.f, 25.f, 26.f};
//     dvdb::cube_888_mask cubes[27], dst;
...
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){ 5,  6,  7}", &dst, cubes, 5, 6, 7);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){-3, -1, -2}", &dst, cubes, -3, -1, -2);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){ 5,  6,  0}", &dst, cubes, 5, 6, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){-3, -1,  0}", &dst, cubes, -3, -1, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){ 5,  0,  0}", &dst, cubes, 5, 0, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){-3,  0,  0}", &dst, cubes, -3, 0, 0);
//     BUDGET_BENCHMARK_CASE(dvdb::rotate_refill, "(mask){ 0,  0,  0}", &dst, cubes, 0, 0, 0);
// }
...
TEST_CASE("leaf_neighbors")
{
    utils::nvdb_mmap nvdb_mmap(utils::resource_path("test/src_big.nvdb"));

    converter::nvdb_reader reader;
    BUDGET_BENCHMARK(reader.initialize, const_cast<void *>(nvdb_mmap.grids()[0].ptr));

    dvdb::cube_888_f32 other_neighbors[27];
    dvdb::cube_888_f32 *neighbors[27];
    dvdb::cube_888_mask *neighbors_masks[27];

    const auto key = reader.leaf_key(0);
    reader.leaf_neighbors(key, neighbors, neighbors_masks, nullptr, nullptr);

    BUDGET_BENCHMARK(reader.leaf_neighbors, key, neighbors, neighbors_masks, nullptr, nullptr);
    BUDGET_BENCHMARK_CASE(std::memcpy, "{27 neighbors}", neighbors, other_neighbors, sizeof(neighbors));
}
